...
void ublox_calc_crc(const char *msg, char *buf, int len)
{
   unsigned char crc;
   char txt[12];
...
int ublox_get_ack(int fd, uint8_t *buf)
{
...
void ublox_crc32(uint8_t *buf, int len)
{
   int i;
   int CK_A = 0, CK_B = 0;
   for(i=0;i<len;i++)
   ...
   CK_A &= 0xFF;
   CK_B &= 0xFF;
   //GNSS_PRINT(("crc32: 0x%02x 0x%02x\n",CK_A,CK_B));
   buf[len] = CK_A;
   buf[len+1] = CK_B;
}

int ublox_send_bin(int fd, uint8_t *msg, uint8_t *buf, int len)
{
   memcpy(buf, msg, len);
   ublox_crc32(&buf[2], len-2);
   //pdump((char *)txBuffer, len+2);
   if (write(fd, buf, len+2) != len+2) {
      printf("ERR: ublox write cmd!\n");
      return -1;
   }
   ublox_get_ack(fd, buf);
   return 0;
}
 ...
 * default:
 *
 * WGS84, antenna on, controlled,
 * auto 2D/3D, m, m², m/s, sec, deg,
 * 1 sec cycle
 * uart - 9600
 * GGA, GLL, GSA, GSV, RMC, VTG,
 * GPTXT - info , warning, notice
 * */
...
/* CFG-MSG                              len 3 */
uint8_t cfg_msg[9] = { 0xB5,0x62,0x06,0x01, 3, 0
      , 0xF1    // msg class
      , 0x00    // msg id
      , 1    // msg rate
};
...
/* CFG-NMEA                              len 4 */
uint8_t cfg_nmea[10] = { 0xB5,0x62,0x06,0x17, 4, 0
      , 0     // no filter
      , 0x23  // nmea 2.3
      , 12    // 12 sat/msg
      , 1     // flag compat mode
...
/* cfg rate                               len 6 */
uint8_t cfg_rate[12] = { 0xB5,0x62,0x06,0x08, 6, 0
      , 0xE8  // low
      , 0x03  // 1000ms meas
      , 1     // low
 ...
 */
/* CFG-PRT                                len 20 */
uint8_t cfg_port[26] = { 0xB5,0x62,0x06,0x00, 20, 0
      , 1     // port usb=3, Uart=1
      , 0     // n.u.
      , 0x00
      , 0x00  // txpin, n.u.
      ...
      , 0xC0  // low mode 8/N/1
      , 0x08
      , (115200 & 0x000000FF) // baud
      , (115200 & 0x0000FF00) >> 8
      , (115200 & 0x00FF0000) >> 16
      , (115200 & 0xFF000000) >> 24
      , 0x23  // low inmask
      , 0x00  //         UBX,NMEA,RTCM3
      , 0x00  // low outmask
      , 0x00  //        2 = NMEA
...
#ifdef ROVER_MODE
/* cfg tmode3                               len 40 */
uint8_t cfg_tmode3[46] = { 0xB5,0x62,0x06,0x71, 40, 0
      , 0     // version
      , 0     // n.u.
      , 0x00  // flags no lon lat..
      , 0x00  // flags disabled receiver mode
      , 0, 0, 0, 0 // x lat
      , 0, 0, 0, 0 // y lon
      , 0, 0, 0, 0 // z alt
      , 0, 0, 0    // precise x,y,z
      , 0     // n.u.
      , 0, 0, 0, 0           // fix pos accuracy
      , 0x2C, 0x01, 0, 0    // survey in min duration 300s
      , 0x20, 0x4E, 0, 0    // survey in accuracy 20000 mm in 0.1mm
      , 0, 0, 0, 0, 0, 0, 0, 0  // n.u.
};
#endif
...
int gnss_parse(char *buffer)
{
     ...
     *
      ...
      $GNTXT,01,01,02,LLC=FFFFFFFF-FFFFFFED-FFFFFFFF-FFFFF79E-FFFFFF69*2E
      $GNTXT,01,01,02,ANTSUPERV=AC SD PDoS SR*3E
      $GNTXT,01,01,02,ANTSTATUS=DONTKNOW*2D
      $GNTXT,01,01,02,PF=359*47
     *
     *
     */
...
char *nextNtripStream(int id, double lat, double lon)
{
   ...
   if (id == IGS_SYSTEM) net = igsIpNet;
   else if (id == SAPOS_SYSTEM) net = ntripSaposDe;
   else return (char*)"";
